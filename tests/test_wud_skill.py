...
def wud_skill_test() -> None:
    """ A test to create env, ground truth nsrts, and wud skill nsrt.
    Then execute the nsrts.


    mostly copied from: predicators/tests/envs/test_spot_envs.py
    """
    ...
    ######################################################
    ### TODO create nsrt using WUD skill and add to nsrts
    ######################################################
    ...
    ######################################################
    WUDGrasp = nsrt_name_to_nsrt["WUDGrasp"]
    ######################################################
    ...
    # Check Predicates and Find Table object
    ...
    ######################################################
    # Run run WUD grasp skill from nsrt.
    wud_grasp = WUDGrasp.ground((spot, cube))

    # Sample and run an option to WUD skill.
    option = wud_grasp.sample_option(state, set(), rng)
    # actions: List[Action] = []  # to test pickling
    # assert option.initiable(state)
    # for _ in range(100):  # should terminate much earlier
    #     action = option.policy(state)
    ...
    #     if option.terminal(state):
    #         break
    # TODO run the option.policy(state) via wud skill interface
    ######################################################
    ...
    # The rest is to finish placing the cube on table
