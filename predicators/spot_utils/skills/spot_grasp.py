...
def simulated_grasp_at_pixel(sim_robot: pbrspot.spot.Spot, rot_constraint: math_helpers.Quat, obj_to_be_grasped: pbrspot.body.Body) -> None:
    """Simulated grasping in pybullet.
    
    For now, this is really dumb and just teleports the object into
    the hand. In the near future, a simple thing to add is just making
    sure the hand can IK to the object. In the further future, this 
    function will hopefully be made be much more reasonable in general
    (we probably want to do BEHAVIOR-style simulated grasping)!
    """
